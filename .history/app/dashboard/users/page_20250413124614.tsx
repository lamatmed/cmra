...
const UsersPage = () => {
  ...
  const handleDeleteUser = async (userId: string) => {
    try {
      ...
      toast({ title: "خطأ", description: "تعذر حذف آخر مسؤول.", variant: "destructive" });
    }
  };
  ...
  const generatePDF = () => {
    ...
    // Titre principal
    doc.setFontSize(16);
    doc.text("Liste des membres de l'union sortantsقائمة الأعضاء المغادرين", 14, 20);
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4 text-center">إدارة المستخدمين</h1>
      <Button className="mb-4" onClick={() => generatePDF()}>
        تحميل قائمة الأعضاء PDF
      </Button>
      <input
        type="text"
      ...
      />
      <h2 className="text-lg font-semibold mb-2">إجمالي عدد المستخدمين: {filteredUsers.length}</h2>
      ...
      <div className="flex justify-center mt-4">
        ...
        <span className="mx-4">الصفحة {currentPage}</span>

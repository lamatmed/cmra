...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error("فشل التسجيل ، مستخدم موجود");
      const newUser = await res.json();

      toast({
        title: "التسجيل الناجح",
        description: `أهلًا وسهلًا ${newUser.name}!`,
      });
      router.push("/login");
    } catch (error: any) {
      toast({
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-between">
              ...
              {step < 3 ? (
                ...
                <Button
                  type="submit"
                  className="w-full bg-green-600 hover:bg-green-700 text-white space-x-1"
                  disabled={loading}
                >

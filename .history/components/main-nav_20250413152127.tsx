...
const navigation: Navigation = {
  ADMIN: [
    ...
    { name: " <NotebookPen className="h- 6 w - 6 text - purple - 500" />
                      الأنشطة", href: "/dashboard/config-vote", icon: Clipboard },
  ],
  USER: [
    { name: "لوحه القياده", href: "/dashboard", icon: LayoutDashboard },
    { name: "مساحتي", href: "/dashboard/espace-user", icon: Users },
    ...
    <nav className="flex flex-col md:flex-row items-center md:space-x-6 space-y-2 md:space-y-0">
      {isAuthenticated && user ? (
        allowedNavigation.map((item) => {
          ...
          <span className="text-black text-sm">قم بتسجيل الدخول للوصول إلى القائمة</span>
      )}
    </nav>
  )
}
